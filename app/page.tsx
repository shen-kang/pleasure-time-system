 ...
 export default function Home() {
   ...
   useEffect(() => { if (userId) writeLocal(recordStoragePrefix + userId, records); }, [records, userId]);
   useEffect(() => { if (userId) writeLocal(spendStoragePrefix + userId, spends); }, [spends, userId]);
   ...
   return (
     <>
       ...
       */}
             ...
             <div className="flex gap-2">
               ...
               <div className="flex gap-1">
                 {colorPalette.slice(0, 8).map(c => (
                   <button key={c} onClick={() => setNewCatColor(c)} className={`h-8 w-8 rounded-full border-2 ${newCatColor === c ? "border-ink" : "border-transparent"}`} style={{ backgroundColor: c }} />
                 ))}
               </div>
               <button onClick={handleAddCategory} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-aqua text-white"><Plus size={18} /></button>
             </div>
       ...
       ===== ===== ===== ===== =====
       */}
       <main className="safe-bottom mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-4 px-4 pb-6 pt-16 sm:px-6 lg:grid lg:grid-cols-[1fr_1.25fr] lg:items-start lg:gap-6 lg:pt-20">
         ...
         <section className="flex flex-col gap-4">
           <div className="rounded-[28px] bg-ink p-5 text-white shadow-soft">
             <div className="mb-3 flex items-center justify-between">
               ...
               <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/12"><WalletCards size={24} /></div>
             </div>
           ...
           */}
           <Panel title="消耗娱乐时间" icon={<Flame size={20} />}>
             <div className="flex gap-3">
               ...
               <button onClick={addSpend} className="h-14 rounded-2xl bg-coral px-5 font-semibold text-white">确认</button>
             </div>
           </Panel>
         </section>
             ...
             <div className="flex flex-col gap-3">
               {records.length === 0 ? (
                 ...
                 <div key={item.id} className="grid grid-cols-[1fr_auto] gap-3 rounded-3xl border border-line bg-white p-4">
                   <div>
                     <div className="flex items-center gap-2">
                       <span className="h-3 w-3 rounded-full" style={{ backgroundColor: colorByCat(item.category) }} />
                       <p className="font-semibold">{item.category}</p>
                       <p className="text-sm text-slate-500">{format(parseISO(item.created_at), "MM/dd HH:mm")}</p>
                     </div>
                     <p className="mt-2 text-sm text-slate-600">{item.hours}小时{item.minutes}分钟 · 专注 {item.focus_score} · {item.points} 积分 · +{item.earned_minutes} 分钟</p>
                   </div>
                   <button aria-label="删除记录" onClick={() => deleteRecord(item.id)} className="flex h-11 w-11 items-center justify-center rounded-2xl bg-mist text-slate-500"><Trash2 size={18} /></button>
                 </div>
               ))}
             </div>
           </Panel>
